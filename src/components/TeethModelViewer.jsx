...
export default function TeethModelViewer({ 
  ...
  const prevViewModeRef = React.useRef(viewMode);
    const prevSelectedRecordKeyRef = React.useRef("__none__");
        const prevDefaultTreatmentRef = React.useRef((defaultTreatment || "").toLowerCase().trim());
  ...
  const updateAnimations = React.useCallback(() => {
    ...
    const keepOnlySelectedArchParts = (object3d, keepUpperArch) => {
        ...
        const isKnownDentalToothId = (id) => isUpperToothId(id) || isLowerToothId(id);
        ...
        object3d.traverse(child => {
            ...
            if (childToothId !== null && isKnownDentalToothId(childToothId)) {
  ...
  React.useEffect(() => {
    ...
    const isToothMissing = React.useCallback((toothNum) => {
        ...
        const recordMarksToothMissing = (record) => {
            ...
            return isMissingLabel(record.condition) || hasCompletedRemoval;
        };
  ...
  React.useEffect(() => {
    ...
    const normalizedDefaultTreatment = (defaultTreatment || "").toLowerCase().trim();
    const defaultTreatmentChanged = prevDefaultTreatmentRef.current !== normalizedDefaultTreatment;
    ...
    if (selectedRecordChanged) {
        ...
        // Reset per-tooth animation completion flags so timeline record previews replay reliably.
        Object.values(map).flat().forEach(mesh => {
            ...
            mesh.userData.activeOrthodonticType = null;
            clearBraceParts(mesh);
        });
    }

    // Allow treatment animations (like cleaning/whitening) to replay when treatment type changes.
    if (viewMode === 'treatment' && defaultTreatmentChanged) {
        state.whiteningMeshes = [];
        state.removalMeshes = [];
        state.cleaningMeshes = [];
        state.bracesMeshes = [];
    ...
    const setupRemoval = (mesh) => {
        // If removal was already completed in a previous preview, allow replay.
        ...
        if (!state.removalMeshes.includes(mesh)) {
            ...
            // Ensure removal starts from a clean, visible baseline.
            mesh.visible = true;
            mesh.position.copy(mesh.userData.originalPosition || new THREE.Vector3(0, 0, 0));
            mesh.scale.copy(mesh.userData.originalScale || new THREE.Vector3(1, 1, 1));
    ...
    const getCleaningStartColor = (conditionText = '') => {
        const normalized = (conditionText || '').toLowerCase().trim();
        if (normalized.includes('stained')) return COLOR_STAINED_BASE.clone();
        if (normalized.includes('decay')) return COLOR_DECAY.clone();
        if (normalized.includes('cavity')) return COLOR_CAVITY.clone();
        if (normalized.includes('corroded')) return new THREE.Color(0x666666);
        return COLOR_DIM.clone();
    };

    const setupCleaning = (mesh, conditionText = '') => {
        if (!state.cleaningMeshes.includes(mesh) && !mesh.userData.cleaningCompleted) {
            ...
            const startColor = getCleaningStartColor(conditionText);
            
            // Reset animation state
            mesh.userData.cleaningStartTime = undefined;
            mesh.userData.cleaningStartColor = startColor.clone();
            mesh.userData.cleaningCompleted = false;
            mesh.material.color.copy(startColor);
    ...
    // Row-level treatment playback can come either from live odontogram selection
    // OR from an opened treatment record timeline item.
    const activeSelectionSet = new Set([
        ...Array.from(selectedSet),
        ...Array.from(selectedRecordToothSet)
    ]);

    const selectedNonMissingSet = new Set(
        Array.from(activeSelectionSet).filter(id => !isToothMissing(parseInt(id, 10)))
    );
    ...
    const defaultTreatmentText = (defaultTreatment || "").toLowerCase();

    const isBracesTreatmentText = (text) => {
        const normalized = (text || "").toLowerCase().replace(/\s+/g, ' ').trim();
        return normalized.includes('brace');
    };

    const isRetainerTreatmentText = (text) => {
        const normalized = (text || "").toLowerCase().replace(/\s+/g, ' ').trim();
        return normalized.includes('retainer');
    };

    const isOrthodonticTreatmentText = (text) => {
        return isBracesTreatmentText(text) || isRetainerTreatmentText(text);
    };

    const getTreatmentForSelectedTooth = (toothNum) => {
        const toothId = String(toothNum);
        const isExplicitlySelected = selectedSet.has(toothId);

        // If the user is actively selecting teeth in treatment mode,
        // the currently chosen treatment in the parent UI takes precedence.
        if (isExplicitlySelected && defaultTreatment) {
        ...
        if (Array.isArray(toothTreatments)) {
            const found = toothTreatments.find(obj => obj?.toothNumber === toothNum);
            treatment = found?.treatment || null;
        } else if (toothTreatments && typeof toothTreatments === 'object') {
            treatment = toothTreatments[toothNum] || null;
        }

        if (!treatment && selectedRecordToothSet.has(String(toothNum))) {
            treatment = selectedRecord?.treatment || selectedRecord?.procedure || null;
        }
    ...
    const isRemovalTreatmentText = (text) => {
        const normalized = (text || "").toLowerCase().replace(/\s+/g, ' ').trim();
        return normalized.includes('remov') || normalized.includes('extract');
    };

    const shouldPreviewMissingTooth = (toothNum) => {
        if (!allowMissingPreview) return false;
        const toothKey = String(toothNum);
        const inActiveSelection = activeSelectionSet.has(toothKey);
        if (!inActiveSelection) return false;

        const assignedTreatment = getTreatmentForSelectedTooth(toothNum);
        const selectedRecordTreatment = selectedRecord?.treatment || selectedRecord?.procedure || '';
        return isRemovalTreatmentText(assignedTreatment) || isRemovalTreatmentText(selectedRecordTreatment) || isRemovalTreatmentText(defaultTreatment);
    };

    const hasOrthodonticSelectionTreatment = Array.from(selectedNonMissingSet).some(id => {
        const toothNum = parseInt(id, 10);
        return isOrthodonticTreatmentText(getTreatmentForSelectedTooth(toothNum));
    });

    const isOrthodonticRowTreatment =
        isOrthodonticTreatmentText(selectedRecordTreatmentText) ||
        isOrthodonticTreatmentText(defaultTreatmentText) ||
        hasOrthodonticSelectionTreatment;
    ...
    prevDefaultTreatmentRef.current = normalizedDefaultTreatment;
    ...
    allToothIds.forEach(toothNumStr => {
        ...
        const previewMissing = isMissing && shouldPreviewMissingTooth(toothNum);

        if (isMissing && !previewMissing) {
        ...
        const assignedTreatment = getTreatmentForSelectedTooth(toothNum);
        const hasAssignedTreatment = Boolean(assignedTreatment);
        const isEffectivelyMissing = isMissing && !previewMissing;

        meshes.forEach(mesh => {
            mesh.userData.isMissing = isEffectivelyMissing;
            if (viewMode === 'status') {
                mesh.visible = true;

                // When a record is selected, only that record should drive highlights.
                if (hasSelectedRecord && !isInSelectedRecord) {
                    setVisuals(mesh, COLOR_DEFAULT);
                } else if (selectedSet.has(String(toothNum))) {
                ...
                const directRemovalPreview =
                    activeSelectionSet.has(String(toothNum)) &&
                    isRemovalTreatmentText(getTreatmentForSelectedTooth(toothNum) || defaultTreatment || selectedRecord?.treatment || selectedRecord?.procedure || '');

                if (directRemovalPreview && !isEffectivelyMissing) {
                    // Hard precedence: selected removal/extraction records must animate removal,
                    // never fall through to condition shading (black/corroded visuals).
                    clearBraceParts(mesh);
                    setupRemoval(mesh);
                    return;
                }
                
                // Check if tooth is selected (highlighted in odontogram) or already has a stored treatment.
                const isToothSelected = activeSelectionSet.has(String(toothNum)) && !isEffectivelyMissing;
                const isToothActiveForTreatment = (isToothSelected || hasAssignedTreatment) && !isEffectivelyMissing;
                console.log(`Tooth ${toothNum} is active for treatment: ${isToothActiveForTreatment}`);
                ...
                selectedNonMissingSet.forEach(selId => {
                    ...
                    if ((isUpper && selIsUpper) || (isLower && selIsLower)) {
                        // For selected teeth, use the active treatment from the UI first.
                        let t = null;
                        if (selectedSet.has(String(selNum)) && defaultTreatment) {
                            t = defaultTreatment;
                        }

                        if (!t && Array.isArray(toothTreatments)) {
                        ...
                        if (!t && selectedRecordToothSet.has(String(selNum))) {
                            t = selectedRecord?.treatment;
                        }

                        if (t && isBracesTreatmentText(t)) {
                            // Row-level braces: activate the whole selected arch.
                            rowTreatment = 'apply dental braces';
                        }
                        if (t && isRetainerTreatmentText(t)) {
                ...
                if (isToothActiveForTreatment || (rowTreatment === 'apply dental braces') || (rowTreatment === 'apply retainer')) {
                    ...
                    if (!treatment) {
                        if (isToothActiveForTreatment && defaultTreatment) {
                            treatment = defaultTreatment;
                            console.log(`📌 Using defaultTreatment first for selected tooth: "${treatment}"`);
                        }

                        // 1. Get treatment from toothTreatments or selectedRecord
                        if (!treatment && toothTreatments && typeof toothTreatments === 'object') {
                            if (Array.isArray(toothTreatments)) {
                    ...
                    if (!treatment && defaultTreatment && isToothActiveForTreatment) {
                        treatment = defaultTreatment;
                    ...
                    if (treatment) {
                        ...
                        // Drive primary animation by selected treatment first.
                        if (normalizedTreatment.includes('whitening') || normalizedTreatment.includes('bleaching')) {
                            console.log(`Selected treatment is whitening/bleaching → WHITENING ANIMATION`);
                            setupWhitening(mesh);
                        }
                        else if (normalizedTreatment.includes('cleaning') || normalizedTreatment.includes('clean') || normalizedTreatment.includes('cavity cleaning')) {
                            console.log(`Selected treatment is cleaning → CLEANING ANIMATION`);
                            setupCleaning(mesh, normalizedCondition);
                        }
                        ...
                        else if (normalizedTreatment.includes('remov') || normalizedTreatment.includes('extract')) {
                        ...
                        else if (isBracesTreatmentText(normalizedTreatment)) {
                            // Check arch selection again to be absolutely sure
                            if ((isUpper && hasUpperSelected) || (isLower && hasLowerSelected)) {
                                if (mesh.userData.activeOrthodonticType !== 'braces') {
                                    clearBraceParts(mesh);
                                }
                                mesh.userData.activeOrthodonticType = 'braces';
                        ...
                        // Apply Retainers visual
                                else if (isRetainerTreatmentText(normalizedTreatment)) {
                             // Check arch selection again to be absolutely sure
                             if ((isUpper && hasUpperSelected) || (isLower && hasLowerSelected)) {
                                if (mesh.userData.activeOrthodonticType !== 'retainer') {
                                    clearBraceParts(mesh);
                                }
                                mesh.userData.activeOrthodonticType = 'retainer';
                        ...
                        else if (isToothActiveForTreatment) {
                            // Default to cleaning animation for any treatment-active tooth.
                            console.log(`Default animation for treatment-active tooth: "${treatment}" → CLEANING ANIMATION`);
                            setupCleaning(mesh, normalizedCondition);
                        } else {
